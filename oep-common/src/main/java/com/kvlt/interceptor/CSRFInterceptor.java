package com.kvlt.interceptor;

import com.alibaba.fastjson.JSONObject;
import com.kvlt.annotation.RefreshCSRFToken;
import com.kvlt.annotation.VerifyCSRFToken;
import com.kvlt.pojo.ResultCode;
import com.kvlt.utils.CSRFTokenUtil;
        ...
        if (verifyCSRFToken != null) {
            ...
            if (verifyCSRFToken.verify() && !verifyCSRFToken(request)) {
                if (StringUtil.isEmpty(xrq)) {
                    ...
                    String csrftoken = CSRFTokenUtil.generate(request);
                    request.getSession().setAttribute("CSRFToken", csrftoken);
                    ResultCode rc = Constant.CSRF_ERROR;
                    response.setContentType("application/json;charset=UTF-8");
                    PrintWriter out = response.getWriter();
